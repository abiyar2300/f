...
import fa.choices
from fa import face_landmarker, state_manager, wording
from fa.common_helper import calc_float_step
from fa.types import FaceLandmarkerModel, Score
...
def render() -> None:
	...
	FACE_LANDMARKER_MODEL_DROPDOWN = gradio.Dropdown(
		...
		choices = fa.choices.face_landmarker_models,
	...
	FACE_LANDMARKER_SCORE_SLIDER = gradio.Slider(
		...
		step = calc_float_step(fa.choices.face_landmarker_score_range),
		minimum = fa.choices.face_landmarker_score_range[0],
		maximum = fa.choices.face_landmarker_score_range[-1]
	)
